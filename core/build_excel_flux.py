#!/usr/bin/env python3
"""
Flux — Multi-Market Weekly Performance Excel Report Builder
Generates one Excel per market (AU, UK, DE, US, CA)
"""
import json, re, os, shutil
import csv as _csv
import re as re2
from pathlib import Path
from datetime import datetime, date, date as _date
...
# ════════════════════════════════════════════════════════════════════════════
# Main function: build Excel for one market
# ════════════════════════════════════════════════════════════════════════════
def build_market_excel(html_path, output_path, sb_csv_path, products_csv_path, title_prefix, market):
    """
    Build a full Excel report for a single Flux market.

    Args:
        html_path:         Path to the generated HTML file
        output_path:       Path to save the Excel file
        sb_csv_path:       Path to SB_report.csv
        products_csv_path: Path to Products.csv
        title_prefix:      e.g. "Flux AU"
        market:            e.g. "AU"
    """
    ...
    c = ws1["A1"]
    c.value = f"{title_prefix} · Weekly Performance Report  |  Feb–Mar 2026  |  W1–W{NW}  |  TACOS Target ≤{tacos_target}%"
    ...
    def total_col(metric):
        return 3 + NW*4 + metric

    TOTAL_COLS = total_col(0)
    ...
    c = ws2["A1"]
    c.value = f"Weekly Raw Data — {title_prefix} — All Metrics per Product per Week  (Source: ScaleInsight + Amazon Ads Console)"
    ...
    c = ws3["A1"]
    c.value = f"Child ASIN Detail — {title_prefix} — Weekly Sales & Spend Breakdown"
    c.font = Font(name="Arial", bold=True, size=11, color="0f172a")
    c.fill = hex_fill("#f1f5f9"); c.alignment = LEFT
    ...
    c = ws4["A1"]
    c.value = f"Week-over-Week Change — {title_prefix}  |  {NW-1} Comparison Periods  |  Green = Improvement, Red = Decline"
    c.font = Font(name="Arial", bold=True, size=11, color="0f172a")
    c.fill = hex_fill("#f1f5f9"); c.alignment = LEFT
    ...
    if os.path.exists(products_csv_path):
        with open(products_csv_path, encoding="utf-8-sig") as f:
    ...
    sb_rows = []
    SALES_COL_NAME = None
    if os.path.exists(sb_csv_path):
        with open(sb_csv_path, encoding="utf-8-sig") as f:
            for r_row in _csv.DictReader(f):
    ...
    c.value = (f"Sponsored Brands Attributed Purchases — {title_prefix} — W1–W{len(_WEEK_RANGES)}  |  "
    ...
    c = ws6["A1"]
    c.value = f"產品象限分析 — {title_prefix}  |  Sales Median = ${med:,.0f}  |  TACOS Target ≤{tacos_target}%  |  {NW}W Total"
    c.font = Font(name="Arial", bold=True, size=12, color="0f172a")
    c.fill = hex_fill("#f1f5f9"); c.alignment = LEFT
    ...
    c = ws7["A1"]
    c.value = f"行動方案清單  |  {title_prefix}  |  W{NW} {w_labels[weeks[-1]]}  |  共 {sum(len(v) for v in quad_counts.values())} 個產品"
    c.font = Font(name="Arial", bold=True, size=12, color="0f172a")
    c.fill = hex_fill("#f1f5f9"); c.alignment = LEFT
...
# ════════════════════════════════════════════════════════════════════════════
# Main: loop through all markets
# ════════════════════════════════════════════════════════════════════════════
if __name__ == "__main__":
    for mkt_key, mkt_label in MARKETS.items():
        html_path = BASE / "output" / "flux" / f"{mkt_key}.html"
        if not html_path.exists():
            print(f"  ⏭ Skipping {mkt_label} — no HTML at {html_path}")
            continue

        filename = f"20260308_Flux_{mkt_label}_Feb-Mar_2026.xlsx"
        output_path = BASE / "output" / "flux" / filename
        copy_path   = BASE / "flux" / filename
        sb_csv_path = BASE / "clients" / "flux" / "input" / mkt_key / "SB_report.csv"
        products_csv_path = BASE / "clients" / "flux" / "input" / mkt_key / "Products.csv"
        title_prefix = f"Flux {mkt_label}"

        print(f"▶ Building {title_prefix} …")
        build_market_excel(
            html_path=str(html_path),
            output_path=str(output_path),
            sb_csv_path=str(sb_csv_path),
        ...
        if copy_path.parent.exists():
            ...
            print(f"  ⚠ flux/ dir not found, skipped copy")
