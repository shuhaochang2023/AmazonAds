...
def build_data(weekly: dict, child_to_parent: dict, asin_to_name: dict,
    ...
    for parent, children in parent_children.items():
        ...
        # Frozen weeks: use preserved parent-level data, but patch SB spend
        ...
        for w in FROZEN_WEEKS:
            ...
            # Patch SB spend if it was 0 (old bug) and we have sb_spend_weekly data
            if wdata.get("sbsp", 0) == 0 and sb_spend_weekly.get(w, 0) > 0:
                total_sb_spend_wk = sb_spend_weekly[w]
                total_sb_attr_wk = sum(sb_attr_by_parent_week[p][w] for p in parent_children)
                if total_sb_attr_wk > 0:
                    sbsp = total_sb_spend_wk * (sb_attr_by_parent_week[parent][w] / total_sb_attr_wk)
                    wdata["sbsp"] = round(sbsp, 2)
                    wdata["spend"] = round(wdata.get("spsd", 0) + sbsp, 2)
                    sales = wdata.get("sales", 0)
                    wdata["tacos"] = round(wdata["spend"] / sales * 100, 1) if sales > 0 else None
            weeks_obj[w] = wdata
