...
TACOS_TARGET  = 10            # % threshold for TACOS classification
...
if __name__ == "__main__":
    ...
    if not PARENT_COLORS:
        _auto_colors = ["#3b82f6","#a855f7","#f59e0b","#22c55e","#ef4444",
        ...
        for i, p in enumerate(unique_parents):
            PARENT_COLORS[p] = _auto_colors[i % len(_auto_colors)]
            PARENT_SHORTS[p] = asin_to_name.get(p, p)[:30]
        print(f"  Auto-detected {len(unique_parents)} parent ASINs from Products.csv")

    # 2. Load frozen data (skip if no frozen weeks or no SOURCE_HTML)
    if FROZEN_WEEKS and SOURCE_HTML.exists():
    ...
    # Use SOURCE_HTML if it exists, otherwise fall back to DAIKEN template
    _template = SOURCE_HTML if SOURCE_HTML.exists() else Path("/Users/koda/amazon-autopilot/clients/daiken/input/DAIKEN_Dashboard_Feb2026.html")
